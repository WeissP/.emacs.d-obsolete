...
#+begin_src elisp
  ...
  (defun weiss--ahf-indent ()
    ...
    (indent-region (- (point) 50) (+ (point) 20))
    t)
...
#+begin_src elisp
  ...
  (define-abbrev-table 'sql-mode-abbrev-table
    '(
      ...
      ("as" "AS")
      ("from" "FROM")
      ("jat" "ALTER TABLE ▮" weiss--ahf)
      ("jav" "AVG(▮)" weiss--ahf)
      ("jct" "COUNT(*▮)" weiss--ahf)
      ("jdi" "DISTINCT")
      ("jdt" "DROP TABLE ▮" weiss--ahf)
      ("jex" "EXISTS (\n▮\n)" weiss--ahf)
      ("jgb" "GROUP BY")
      ("jin" "INSERT INTO ▮" weiss--ahf)
      ("jinv" "INSERT INTO ▮ VALUES()" weiss--ahf)
      ("jli" "LIMIT 10" weiss--ahf)
      ("jma" "MAX(▮)" weiss--ahf)
      ("jmi" "MIN(▮)" weiss--ahf)
      ("jnt" "CREATE TABLE ▮ \n(\n\n)" weiss--ahf)
      ("jnv" "CREATE VIEW ▮ AS\n(\n\n)" weiss--ahf)
      ("jnf" "CREATE OR REPLACE FUNCTION ▮ () RETURNS  AS $$\nDECLARE\nBEGIN\n\nRETURN;\nEND; $$ LANGUAGE plpgsql;\n" weiss--ahf)
      ("jntr" "CREATE TRIGGER ▮ \nBEFORE INSERT ON \nFOR EACH ROW \nEXECUTE PROCEDURE ;" weiss--ahf)
      ("job" "ORDER BY")
      ("jov" "OVER (▮) AS" weiss--ahf)
      ("jpb" "PARTITION BY")
      ("jsf" "SELECT \nFROM ▮")
      ("jsfw" "SELECT \nFROM ▮\nWHERE ")
      ("jsl" "SELECT")
      ("jst" "SELECT * FROM ▮ LIMIT 10" weiss--ahf)
      ("jun" "UNION")
      ("jua" "UNION ALL")
      ("jwh" "WHERE")
      ("jrn" "RAISE NOTICE '▮'" weiss--ahf)
      ("lj" "NATURAL JOIN")
